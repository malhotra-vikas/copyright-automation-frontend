...
import { CheckCircle, FileText, RefreshCw, ExternalLink, X } from "lucide-react";
...
export default function CopywriterTaskList() {
  ...
  const savePrompt = async () => {
    try {
      ...
      setRecords((prev) =>
        prev.map((r) =>
          r.id === record.id
            ? { ...r, fields: { ...r.fields, [`${editablePromptType}-prompt`]: editablePrompt } }
            : r
        )
      );
  ...
  return (
    <div className="w-full py-6 px-6 h-screen flex flex-col">
      ...
      <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
        <div className="flex gap-2 flex-wrap">
          <Button
            size="sm"
            variant="outline"
            onClick={() => toast.success("✅ Approved and Sent!")}
          >
        ...
        <div className="flex gap-2 items-center">
          ...
          <span className="text-sm text-gray-500">
            Email {currentIndex + 1} of {records.length}
          </span>
          <Button
            variant="outline"
      ...
      <div className="flex flex-grow gap-6 overflow-hidden">
        ...
        <Card className="flex-1 min-h-full">
          <CardHeader>
            <CardTitle>Email to {record.fields["First Name"]}</CardTitle>
          </CardHeader>
          ...
          <CardContent className="space-y-2 text-sm text-muted-foreground">
            ...
            <p>
              ...
              <a
                ...
                href={`https://${record.fields["Website"]}`}
                target="_blank"
              >
                {record.fields["Website"]} <ExternalLink className="inline h-4 w-4 ml-1" />
              </a>
            </p>
            <p>
              <strong>Slack:</strong>{" "}
              <a className="text-blue-600" href={record.fields["client slack"]} target="_blank">
                Slack Link
              </a>
            </p>
            <p>
              <strong>Onboarding Doc:</strong>{" "}
              <a
                className="text-blue-600"
                href={record.fields["client onboarding doc"]}
                target="_blank"
              >
                View Doc
              </a>
      ...
      {visiblePrompt !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-2xl p-6 rounded shadow-xl relative">
            ...
            <h2 className="text-lg font-semibold mb-3">Edit Prompt</h2>
            <textarea
              className="w-full border p-3 rounded text-sm text-gray-700 mb-4"
              rows={6}
